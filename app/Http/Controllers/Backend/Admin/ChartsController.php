    ...
    {  
        ...
        DB::table('charts')
                ...
                ->update([
                   ...
                   // 'order' => $order,
                    'content' =>$request->post('content'),
                    'chart_options' =>$request->post('chart_options'),
   ...
   public function get_score(Request $request)
    {
       ...
       for ($i=1;$i< count($questions);$i++){
        ...
        if(count($qdata2)!==0){
            ...
            // foreach ($qdata2 as $key=>$value) {
            //     $arr[$value->test_id][]= array(
            //         'score'=>$value->score,
            //     );
            // }
            print_r($qdata2);
       ...
       for ($i=1;$i< count($questions);$i++){
        ...
        foreach ($arr as $value) {
           ...
           foreach($value as $values){
            ///echo $values['score'][0];
            // $newvar = str_replace('[', '', $values['score']);
            // $newvar1 = str_replace('null',0,str_replace(',','',str_replace('"','',str_replace(']', '', $newvar))));
                
            //     // $total = (int)$newvar1+(int)$total; 
    ...
    //     $question_infos = DB::table('questions')
    //         ->select('questions.id','questions.questiontype','questions.question','questions.content')
