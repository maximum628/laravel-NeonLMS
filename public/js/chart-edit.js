var ChartEdit = function () {
    ...
    var ExpressionCalculation = function(){
        ...
        $.ajax({
            ...
            success: function(response){
                ...
                {
                    if(response['qdata'][i].length == 0) continue;
    ...
    var chart_options_serialize = function (){
        var return_options = {
            
        };
        var charttype =$( "#chart_type option:selected").text();
        return_options["type"] = charttype;
        if(charttype == "bar"){
            return_options["options"] = {};
            return_options["options"]['orientation']= $( "#bar_chart_orientation option:selected").text();
            return_options["options"]['stacked']= document.getElementById("bar_stacked").checked;
            return_options["options"]['bgcolor']=  $("#bar_chart_color").val();
        }
    ...
    $("#create_chart").on('click',function(){
        ...
        switch(selected_text)
        {
            case "bar":
                {       
                    chart_draw_by_type('bar',content, "");
                    // pie_chart_draw1(content, "");
                    $('#pie-chartdiv').show();
                    break;
                }
            case "donut chart":
    ...
    $("#chart_type").on('change',function(){
        console.log($("#chart_options").serialize());
        var charttype =$( "#chart_type option:selected").text();
        $('.chart_option').hide();
        $('#'+charttype+'_options').show();
    });
    ...
    $('#save_data').on('click',function(e){
        ...
        console.log(chart_options_serialize(),JSON.stringify(chart_options_serialize()));
        data = {
            'id':$("#chart_id").val(),
            'title':$("#title").val(),
            ...
            'chart_options' : JSON.stringify(chart_options_serialize())
    ...
    var chart_option_setting = function(options){
        if(options.type == 'bar'){
            $( "#bar_chart_orientation").val(options.options.orientation);
            document.getElementById("bar_stacked").checked = options.options.stacked;
            $("#bar_chart_color").val(options.options.bgcolor);
        }
    }
    ...
    return {
        ...
        init: function () {
            chart_option_setting(chart_option);
